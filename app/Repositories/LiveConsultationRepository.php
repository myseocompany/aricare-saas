...
class LiveConsultationRepository extends BaseRepository
{
    ...
    public function store(array $input): bool
    {
        ...
        try {
            ...
            $input['time_zone'] = $input['time_zone'] ?? '';
            ...
            if (! empty($hospitalDefaultAdmin)) {
                ...
                Mail::to($doctor->user->email)
                    ->send(new NotifyMailLiveConsultation(
                        'emails.live_consultation_created_mail',
    ...
    {
        if ($status == 'all') {
            return LiveConsultation::where(
                'patient_id',
                getLoggedInUser()->owner_id
            )->with('patient', 'doctor', 'user')->orderBy('id', 'desc')->get();
            ...
            return LiveConsultation::with('patient', 'doctor', 'user')->where(
                'status',
                LiveConsultation::STATUS_CANCELLED
            )->where('patient_id', getLoggedInUser()->owner_id)->orderBy('id', 'desc')->get();
        } elseif ($status == 'finished') {
            return LiveConsultation::with('patient', 'doctor', 'user')->where(
                'status',
                LiveConsultation::STATUS_FINISHED
            )->where('patient_id', getLoggedInUser()->owner_id)->orderBy('id', 'desc')->get();
        } else {
            return false;
        }
    ...
    public function googleMeetStore($input)
    {
        try {
            ...
            $input['status'] = $input['status'] ?? 0;
            $input['meeting_id'] = $input['meeting_id'] ?? '';
            $input['time_zone'] = $input['meeting_id'] ?? '';
