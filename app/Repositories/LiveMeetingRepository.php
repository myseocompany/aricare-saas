...
class LiveMeetingRepository extends BaseRepository
{
        ...
        try {
            ...
            if (! empty($hospitalDefaultAdmin)) {
                ...
                Mail::to(getLoggedInUser()->email)
                    ->send(new NotifyMailLiveMeeting(
                        'emails.live_meeting_created_mail',
    ...
    public function edit(array $input,  $liveMeeting): bool
    {
        try {
            /** @var ZoomRepository $zoomRepo */
    ...
    {
        try {
            $roles = User::orderBy('first_name')->whereTenantId(getLoggedInUser()->tenant_id)->whereHas('roles', function (Builder $query) {
