...
class ScheduleRepository extends BaseRepository
{
    ...
    public function getData($operation = null): array
    {
        /** @var User $user */
        $user = Auth::user();
        $data = [];
        ...
        /** @var Doctor $doctors */
        if ($operation == 'create') {
            $query = Doctor::with('doctorUser')->where('tenant_id', getLoggedInUser()->tenant_id)->withCount('schedules')->having('schedules_count', '==', 0);
        } else {
            $query = Doctor::with('doctorUser')->where('tenant_id', getLoggedInUser()->tenant_id);
        }
